...
//获取消息页面
function getResourcePDFPage(){	
	$.post("getResourceObject.do"+search,null,function(result){
		console.log(JSON.stringify(result));
		if(result.state==1){
			setResourcePDFInfo(result.data.resource,result.data.valid);
		}else if(result.state==0){
...
//设置消息页面
function setResourcePDFInfo(resource,valid){
	//1.设置标题
	//2.设置副标题
    ...
    PDFObject.embed(resource.path, ".pdf",options);
}